...
const STOP_WORDS = new Set(["a","an","the","and","or","is","it","in","on","at","to","of","for","was","were","has","have","with","this","that","my","i","me","been"])
const keywords = (s = "") => norm(s).split(" ").filter(w => w.length > 2 && !STOP_WORDS.has(w))
const sharedKeywordCount = (a, b) => {
  const sa = new Set(keywords(a)), sb = new Set(keywords(b))
...
const loadCache = () => { try { const c = localStorage.getItem(CACHE_KEY); return c ? JSON.parse(c) : [] } catch { return [] } }
const saveCache = (items) => { try { localStorage.setItem(CACHE_KEY, JSON.stringify(items)) } catch {} }
...
function Dashboard({ darkMode }) {
  ...
  const deleteItem = useCallback(async (id) => {
    ...
    setItems(updated); saveCache(updated)
    try { await apiDelete(id) } catch { setItems(prev); saveCache(prev) }
  }, [])

  const resolveItem = useCallback(async (id) => {
    const prev = itemsRef.current
    const updated = prev.map(item => item.id === id ? { ...item, status: "Resolved" } : item)
    setItems(updated); saveCache(updated)
  ...
  const editItem = useCallback(async (updatedItem) => {
    ...
    setItems(updated); saveCache(updated)
    try { await apiUpdate(updatedItem) } catch { setItems(prev); saveCache(prev) }
  }, [])
  ...
  const findMatchesWithAI = useCallback(async (currentItems) => {
    ...
    for (const lost of lostItems) {
      const candidates = foundItems.filter(f => locationsMatch(f.location, lost.location))
      for (const found of candidates) {
        const pairKey = [lost.id, found.id].sort().join("|")
        if (seenPairs.has(pairKey)) continue
        seenPairs.add(pairKey)
        let score = 0, maxScore = 0
        maxScore += 2; if (namesAreRelated(lost.name, found.name)) score += 2
        maxScore += 1; if (locationsMatch(lost.location, found.location)) score += 1
        if (lost.category && found.category) { maxScore += 1; if (norm(lost.category) === norm(found.category)) score += 1 }
        if (lost.description && found.description) { maxScore += 1; if (sharedKeywordCount(lost.description, found.description) >= 1) score += 1 }
        if (lost.image && found.image) {
          maxScore += 2
          try {
            if (!detectedCache.current[lost.id]) detectedCache.current[lost.id] = await detectObject(lost.image)
            if (!detectedCache.current[found.id]) detectedCache.current[found.id] = await detectObject(found.image)
            const lObj = detectedCache.current[lost.id], fObj = detectedCache.current[found.id]
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (!cancelled) { setMatches(result); if (result.length > 0) setShowMatchAlert(true) }
      } catch {}
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", marginBottom: "28px" }}>
          ...
          <div style={{
            ...
            {syncError ? "Backend offline — showing cached data" : syncing ? "Syncing..." : "Live"}
          </div>
        ...
        <div style={{
          ...
          {items.length === 0 ? (
            <div style={{
              ...
              <h3 style={{ fontSize: "22px", color: t.text, margin: "0 0 8px" }}>No items yet!</h3>
